...
until (
    ...
    nohup accelerate launch \
        ...
        --num_machines $NNODES \
        --machine_rank $GEMINI_CURRENT_TASK_ROLE_CURRENT_TASK_INDEX \
        --main_process_ip $GEMINI_HOST_IP_taskrole1_0 \
        ...
        --logdir ltx_experiments/0519_5nodes_bs200_causaldmd \
        > $LOGFILE 2>&1 &

    PID=$!
    echo "[`date +'%F %T'`] 启动训练脚本，PID=$PID"
    wait $PID  # 等待该后台进程结束，$? 即为退出码
); do
    EXIT_CODE=$?
