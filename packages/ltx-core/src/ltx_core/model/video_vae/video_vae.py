...
def map_temporal_slice(begin: int, end: int, left_ramp: int, right_ramp: int, scale: int) -> Tuple[slice, torch.Tensor]:
    ...
    # --- 新逻辑：重叠区完全使用前一段的输出 ---
    # 初始化一个全 1 的掩码（保证前一段的右侧重叠区权重为 1.0）
    mask_1d = torch.ones(stop - start)
    
    # left_ramp_frames 代表当前块与“前一段”重叠的部分
    if left_ramp_frames > 0:
        # 将当前块在重叠区的权重强制设为 0，这样它就不会对画面产生任何贡献
        mask_1d[:left_ramp_frames] = 0.0
