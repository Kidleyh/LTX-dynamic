...
class CausalBasicAVTransformerBlock(torch.nn.Module):
    def __init__(
        self,
        idx: int,
        ...
        norm_eps: float = 1e-6,
        attention_function: AttentionFunction | AttentionCallable | None = None,
    ):
        super().__init__()
        ...
        if audio is not None:
            self.audio_attn1 = CausalAttention(
                query_dim=audio.dim,
                heads=audio.heads,
            ...
            self.audio_attn2 = CausalAttention(
                query_dim=audio.dim,
        ...
        if audio is not None and video is not None:
            # Q: Video, K,V: Audio
            self.audio_to_video_attn = CausalAttention(
                query_dim=video.dim,
            ...
            # Q: Audio, K,V: Video
            self.video_to_audio_attn = CausalAttention(
                query_dim=audio.dim,
    ...
    def _apply_text_cross_attention(
        ...
        return attn(rms_norm(x, eps=self.norm_eps), context=context, mask=context_mask, kv_cache=kv_cache, kv_cache_name=kv_cache_name)
    ...
    def forward(  # noqa: PLR0915
        ...
        if run_vx:
            ...
            vx = (
                ...
                + self.attn1(
                    ...
                    kv_cache=video_self_attn_kv_cache,
                    kv_cache_name="video_self_attn_kv_cache",
            ...
            vx = vx + self._apply_text_cross_attention(
                ...
                kv_cache=video_cross_attn_kv_cache,
                kv_cache_name="video_cross_attn_kv_cache",
                kv_cache_snapshot=kv_cache_snapshot,
            )
        ...
        if run_ax:
            ...
            # TODO: This attention waste lots of time, need to optimize!
            ax = (
                ax
                + self.audio_attn1(
                    norm_ax,
                    ...
                    all_perturbed=all_perturbed,
                    kv_cache=audio_self_attn_kv_cache,
                    kv_cache_name="audio_self_attn_kv_cache",
                    kv_cache_snapshot=kv_cache_snapshot,
                )
                * agate_msa
            )
            ...
            ax = ax + self._apply_text_cross_attention(
                ...
                audio.context_mask,
                cross_attention_adaln=self.cross_attention_adaln,
                kv_cache=audio_cross_attn_kv_cache,
                kv_cache_name="audio_cross_attn_kv_cache",
                kv_cache_snapshot=kv_cache_snapshot,
            )
            ...
            if run_a2v and not perturbations.all_in_batch(PerturbationType.SKIP_A2V_CROSS_ATTN, self.idx):
                ...
                vx = vx + (
                    self.audio_to_video_attn(
                        ...
                        mask=video.multimodel_attention_mask,
                        kv_cache=a2v_cross_attn_kv_cache,
                        kv_cache_name="a2v_cross_attn_kv_cache",
                        kv_cache_snapshot=kv_cache_snapshot,
                    )
                    * gate_out_a2v
                    * a2v_mask
            ...
            if run_v2a and not perturbations.all_in_batch(PerturbationType.SKIP_V2A_CROSS_ATTN, self.idx):
                ...
                ax = ax + (
                    self.video_to_audio_attn(
                        ...
                        mask=audio.multimodel_attention_mask,
                        kv_cache=v2a_cross_attn_kv_cache,
                        kv_cache_name="v2a_cross_attn_kv_cache",
                        kv_cache_snapshot=kv_cache_snapshot,
                    )
                    * gate_out_v2a
                    * v2a_mask
...
def apply_cross_attention_adaln(
    ...
    return attn(attn_input, context=encoder_hidden_states, mask=context_mask, kv_cache=kv_cache, kv_cache_name=kv_cache_name, kv_cache_snapshot=kv_cache_snapshot) * q_gate
