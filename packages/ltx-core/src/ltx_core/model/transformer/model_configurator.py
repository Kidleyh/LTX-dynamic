...
class CausalLTXModelConfigurator(ModelConfigurator[LTXModel]):
    """
    Configurator for LTX model.
    Used to create an LTX model from a configuration dictionary.
    """

    use_flex_attention: bool = True  # AttentionFunction.DEFAULT

    @classmethod
    def from_config(cls: type[CausalLTXModel], config: dict) -> CausalLTXModel:
        from ltx_core.model.transformer.causal_attention import AttentionFunction
