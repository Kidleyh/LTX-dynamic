...
class AVCausalMaskBuilder:
    ...
    def build_video_self_causal_mask(
        ...
        Uses PyTorch SDPA instead of FlexAttention. 
        Modified to use Sink + Sliding Window rules (Anchor: 0, 1; Window: i, i-1, i-2).
        """
        ...
        for i, q_block in enumerate(blocks):
            q_start = q_block.video_start * self.video_frame_seqlen
            q_end = q_block.video_end * self.video_frame_seqlen
            
            # 定义允许的 kv_block 索引：全局锚点 (0, 1) + 自身 (i) + 前向滑窗 (i-1, i-2)
            allowed_kv_blocks = {0, i, i - 1, i - 2}
            
            for j in allowed_kv_blocks:
                if 0 <= j < len(blocks):
                    kv_block = blocks[j]
                    kv_start = kv_block.video_start * self.video_frame_seqlen
                    kv_end = kv_block.video_end * self.video_frame_seqlen
                    
                    # 激活 Query Token 区域与允许的 KV Token 区域之间的 Attention
    ...
    def build_audio_self_causal_mask(
        ...
        for i, q_block in enumerate(blocks):
            if q_block.audio_end <= q_block.audio_start:
                continue
                
            q_start = q_block.audio_start * self.audio_frame_seqlen
            q_end = q_block.audio_end * self.audio_frame_seqlen
            ...
            for j in allowed_kv_blocks:
                if 0 <= j < len(blocks):
                    kv_block = blocks[j]
                    if kv_block.audio_end <= kv_block.audio_start:
                        continue
                        
                    kv_start = kv_block.audio_start * self.audio_frame_seqlen
                    kv_end = kv_block.audio_end * self.audio_frame_seqlen
...
def build_all_causal_masks(
    ...
    # import cv2
    # cv2.imwrite("test_new_video_self_mask.jpg", masks['video_self'].int().cpu().numpy()*255)
    # cv2.imwrite("test_new_audio_self_mask.jpg", masks['audio_self'].int().cpu().numpy()*255)
    # cv2.imwrite("test_new_a2v_self_mask.jpg", masks['a2v'].int().cpu().numpy()*255)
    # cv2.imwrite("test_new_v2a_self_mask.jpg", masks['v2a'].int().cpu().numpy()*255)
