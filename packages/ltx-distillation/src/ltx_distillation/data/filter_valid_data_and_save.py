import sys
sys.path.insert(0, "/gemini-1/space/human_guozz2/code/hys/videoxfun_0711/videox_fun/data")
...
class AudioVisualDataset(Dataset):
    def __init__(
        ...
        # data_path_list = dataset_paths_txt
    ...
    # overwrite filter
    def filter_and_save_data(self, raw_clip_dataset, save_path):
        ...
        for c in tqdm(valid_data_list_ai2v, desc="copying valid videos:"):
            clip_dataset, clip_name = c.file_path.split(":")
            clip_name = clip_name.split("/")[-1]
            dataset_name = clip_dataset.split("/")[-4]
            new_clip_name = f"{dataset_name}@@{clip_name}"
            # print(c.file_path, dataset_name, clip_name)
            ori_clip_path = os.path.join(clip_dataset, clip_name)
            new_clip_path = os.path.join(save_path, new_clip_name)
            if os.path.exists(new_clip_path):
                continue
            shutil.copy(ori_clip_path, new_clip_path)

        return valid_data_list_ai2v, valid_data_list_i2v

def main():
    dataset_names = ["openhuman_vid_007",
                     "openhuman_vid_009", "openhuman_vid_010", 
                     "openhuman_vid_016",
                     "openhuman_vid_017", "openhuman_vid_018", 
                     "openhuman_vid_019", "openhuman_vid_020"]
    for dataset_name in dataset_names:
        dataset_path_list = [f"/gemini-1/space/human_guozz2/data/wzh/omnihuman_dataset/open_source_dataset/openhumanvid/{dataset_name}/config_hys_gather.json"]
        output_video_path = f"/gemini-1/space/human_guozz2/data/wzh/omnihuman_dataset/valid_for_annotation_hys/{dataset_name}"

    # dataset_names = ["pack_20_2_new", "pack_20_1_1", "pack_20_1_2"]
    # for dataset_name in dataset_names:
    #     dataset_path_list = [f"/gemini-1/space/human_guozz2/data/wzh/omnihuman_dataset/general_data_1/{dataset_name}/config_hys_gather.json"]
    #     output_video_path = f"/gemini-1/space/human_guozz2/data/wzh/omnihuman_dataset/valid_for_annotation_hys/{dataset_name}"

        os.makedirs(output_video_path, exist_ok=True)
        dataset = AudioVisualDataset(video_sample_stride=1, video_sample_n_frames=81)
        raw_clip_dataset = dataset.load_data_list(dataset_path_list)
        _, _ = dataset.filter_and_save_data(raw_clip_dataset, output_video_path)
