...
import debug_image_utils as image_utils
...
class AudioVisualDataset(Dataset):
    def __init__(
        ...
        # dataset
        self.ai2v_dataset = valid_data_list_ai2v
        self.i2v_dataset = valid_data_list_i2v

        self.dataset = self.ai2v_dataset + self.i2v_dataset # ai2v在前

        transforms_dict=[
            dict(
                type="SampleImages",
                num_frames=video_sample_n_frames,
                sample_type="jump_chunks_from_first", # second chunk可能和first chunk相隔一段距离
                stride=video_sample_stride,
            ),
            dict(
            ...
            dict(
                type="GuidanceDropSelector",
            ),

            dict(
                type="PromptGenerator",
                clean_prompt=True,
                short_prompt_prob=0.7,
                default_prompt_prob=-1,
            ),
            # TODO: 还没仔细看
            # dict(
            #     type="PromptToTransformerEmbedding",
            #     model_name="llava",
            #     model_path="/gemini-1/space/songqy/vast_old/pretrained/hunyuanvideo_i2v/text_encoder",
            #     max_length=256,
            ...
            #     model_path="/gemini-1/space/songqy/vast_old/pretrained/hunyuanvideo_i2v/text_encoder_2",
            # ),
            dict(
                type="GenerateChineseWav2vecFeature",
                num_frames=video_sample_n_frames,
            ),
            dict(
                type="SampleFaceBoundingBoxesDummy",
            ),
            # dict(
            #     type="SampleLipMasks"
            # ),
            dict(
                type="PackInputs",
                image_keys=["images", "first_ref_image", "face_masks"],
                embedding_keys=[
                    # "prompt_embeds",
                    # "prompt_masks",
                    # "clip_text_embed",
                    "wav2vec_embedding",
                    # "bg_audio_feature",
                    # "origin_audio_feature",
                ],
                dst_size=dst_size, # [NOTE]: NO use
            ),
        ]

        # transforms_pipeline = build_transform(transforms_dict) # [NOTE]: 这里可能处理的有问题

        # transform
        # self.pipeline = Compose(transforms_dict)

        # # self.length = len(self.dataset)
        # print(f"data scale: ai2v: {self.ai2v_length} i2v: {self.i2v_length}")
    ...
    def load_data_list_debug_lmk(self, data_path_list):
        data_list = []
        for data_path in tqdm(data_path_list):
    ...
    def load_data_list_debug(self, data_path_list):
        data_list = []
        for data_path in tqdm(data_path_list):
            with open(data_path) as f:
                dataset = json.load(f)
        ...
        for clip in raw_clip_dataset:
            ...
            if abs(clip.lip_sync.audio_video_offset) > 5 or \
                clip.lip_sync.LSE_C <= 3 or \
                    clip.lip_sync.LSE_D >= 12:
                print(clip.lip_sync.audio_video_offset, clip.lip_sync.LSE_C, clip.lip_sync.LSE_D)
                audio_filter_cnt += 1
                clip.train_task = "i2v"
            ...
            start_end_indx = self.longest_above_threshold(areas, 150*150) # 128x128 + 90 frame: 1046/15015, 128x128: 1177, 32x32: 1212/15015
            ...
            # size
                ...
                if actual_valid_range_e - actual_valid_range_s < min_num_frames + 10 + 24: # [NOTE] 前后留一个冗余, 以及为了长视频留一段间断冗余
                    video_len_filter_cnt += 1
                    continue

                if (abs(float(clip.width / clip.height) - 1.29) > 0.1) and (abs(float(clip.width / clip.height) - 0.78) > 0.1) and (abs(float(clip.width / clip.height) - 0.57) > 0.1) and (abs(float(clip.width / clip.height) - 1) > 0.1) and (abs(float(clip.width / clip.height) - 1.75) > 0.1) :
                    # [NOTE]: 只保留接近16:9 1:1 9:16, 4:3 3:4的视频
                    ar_ratio_filter_cnt += 1
                    continue

                # if abs(float(clip.width / clip.height) - 1) > 0.1:
                #     # [NOTE]: 只保留接近1:1的视频
                #     ar_ratio_filter_cnt += 1
                #     continue
    ...
    def get_data_info(self, clip):
        ...
        data_dict["audio_valid_range"] = clip.lip_sync.audio_valid_range
        data_dict["audio_feature_path"] = clip.audio_feature_path
        # audio_raw_human_path = "/".join(clip.audio_feature_path.human.split("/")[:-2]) + "/raw/" + os.path.basename(clip.audio_feature_path.human)[:-4] + ".wav"
        # data_dict["audio_raw_human_path"] = audio_raw_human_path
        wav2vec_human_path = "/".join(clip.audio_feature_path.human.split("/")[:-2]) + "/wav2vec_chinese/" + os.path.basename(clip.audio_feature_path.human)[:-4] + ".npy"
        data_dict["chinese_wav2vec_feature_path"] = wav2vec_human_path
        data_dict["lse_c"] = clip.lip_sync.LSE_C
        data_dict["lse_d"] = clip.lip_sync.LSE_D
        data_dict["audio_video_offset"] = clip.lip_sync.audio_video_offset
        data_dict["audio_bbox"] = clip.lip_sync.audio_bbox

        # load face info
        data_dict["track_id"] = clip.track_id
        data_dict["data_path"] = clip.data_path
        data_dict["train_task"] = clip.train_task

        # print(data_dict["train_task"])

        # face info related
        # data_dict["variance_expr"] = clip.face_info.variance_expr
        # data_dict["variance_head"] = clip.face_info.variance_head
        # data_dict["lip_keypoints"] = clip.face_info.lip_keypoints
        # data_dict["lip_kp_conf"] = clip.face_info.lip_kp_conf
    ...
    def get_batch(self, idx):
        ...
        for _ in range(self.max_refetch + 1):
            try:
                clip = copy.deepcopy(self.dataset[idx])
                data_info = self.get_data_info(clip)
    ...
    def __len__(self):
        return self.ai2v_length

    # def get_item_with_task(self, idx, task):
    #     sample = self.get_batch(idx, task)
    #     return sample
...
if __name__ == "__main__":
    train_dataset = AudioVisualDataset(
        ...
        video_sample_size=896, video_sample_stride=1, video_sample_n_frames=81, 
        video_repeat=1, 
        image_sample_size=512,
        enable_bucket=True, enable_inpaint=True,
        dataset_paths_txt="/gemini/platform/public/aigc/human_guozz2/code/hys/longtalker_0319/wan/data/dataset_txts/debug_hn2.txt"
    )