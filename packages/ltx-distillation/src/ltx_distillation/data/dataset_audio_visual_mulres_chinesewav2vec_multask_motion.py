...
class Clip:
    ...
    # motion_variance_head: Optional[List[float]] = None  # 运动方差
    # motion_variance_expr: Optional[List[float]] = None  # 表情方差
...
class AudioVisualDataset(Dataset):
    def __init__(
        ...
        transforms_dict=[
            ...
            dict(
                type="SampleFaceBoundingBoxesDummy", # [NOTE]: for memory saving's sake
            ),
            # dict(
            #     type="SampleLipMasks"
            # ),
    ...
    def load_data_list(self, data_path_list):
        ...
        for data_path in tqdm(data_path_list):
            ...
            variance_dir = os.path.join(dataset_dir, "v0.0.1", "face", "variances_new_2")

            for clip in tqdm(dataset["clips"]):
                temp_clip = copy.deepcopy(clip)
                
                variances_path = os.path.join(variance_dir, str(clip['id'])+".pkl")

                if not os.path.exists(variances_path):
                    # print(f"no variances for {data_path}")
                    continue

                with open(variances_path, "rb") as f:
                ...
                for k in range(len(clip['lip_sync']['LSE_C'])): # track-wise data
                    ...
                    if variance_info[k]['expr_variances'] is None:
                        continue
    ...
    def filter_data(self, raw_clip_dataset):
        ...
        for clip in raw_clip_dataset:
            ...
            # clip_frame_num = len(clip.lip_sync.audio_bbox)
