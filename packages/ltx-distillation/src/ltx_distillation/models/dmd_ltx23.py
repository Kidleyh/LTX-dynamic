...
class LTX23DMD(nn.Module):
    ...
    def _compute_kl_grad(
        ...
        pred_real_uncond_video, pred_real_uncond_audio = self.real_score(
            ...
            # noisy_audio=noisy_audio,
            # audio_timestep=audio_sigma,
            video_modality_uncond,
            audio_modality_uncond,
            perturbations=None,
        )
        ...
        clean_video = video_state.latent
        clean_audio = audio_state.latent
        ...
        if normalization:
            ...
            if video_loss_mask is not None:
                video_mask = video_loss_mask.to(p_real_video.dtype)
            ...
            if audio_loss_mask is not None:
                audio_mask = audio_loss_mask.to(p_real_audio.dtype)
    ...
    def compute_distribution_matching_loss(
        ...
        with torch.no_grad():
            ...
            if self._is_bidirectional_task(self.generator_task_type):
                # video_timestep, audio_timestep, sampled_timestep = self._sample_i2v_timesteps(
                #     B,
                #     video_loss_mask,
                #     audio_loss_mask,
                # )
                ...
                # noise_video = torch.randn_like(video_state.latent)
                # noise_audio = torch.randn_like(audio_state.latent)
                ...
                # noisy_video_state = replace(video_state, latent=noisy_video)
                # noisy_audio_state = replace(audio_state, latent=noisy_audio)
                ...
                # )
                raise NotImplementedError("Causal dmd loss to be implemented")
