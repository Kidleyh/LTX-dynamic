...
class AspectRatioBatchImageVideoSampler(BatchSampler):
    ...
    def __init__(self,
        ...
        self.bucket = {
            'video':{
                4:{ratio: [] for ratio in aspect_ratios},
                5.2:{ratio: [] for ratio in aspect_ratios},
                6.5:{ratio: [] for ratio in aspect_ratios},
                }
        }
    ...
    def __iter__(self):
        for idx in self.sampler:
            try:
                ...
                if video_dict.get("video_train_time", 6.5) != 6.5:
                    continue
            ...
            if closest_ratio not in self.current_available_bucket_keys:
                continue
            train_time = video_dict.get("video_train_time", 6.5)
            bucket = self.bucket['video'][train_time][closest_ratio]
            bucket.append(idx)
            # yield a batch of indices in the same aspect ratio group
            if len(bucket) == self.batch_size:
                yield bucket[:]
