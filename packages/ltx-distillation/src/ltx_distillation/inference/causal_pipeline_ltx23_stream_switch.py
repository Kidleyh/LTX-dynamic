...
class LTX23CausalAVStreamSwitchInferencePipeline:
    """
    ...
    def switch_text_context_dict(self,
                                 block_idx: int, 
                                 prompt_block_num_list: List[int],
                                 text_context_dict_list: List[Dict[str, Any]]):

        if not prompt_block_num_list or not text_context_dict_list:
            return None
            
        current_idx = block_idx
        
        for i, num_blocks in enumerate(prompt_block_num_list):
            # 如果剩余的当前 index 小于该段 prompt 的 block 数量，说明落在了第 i 段
            if current_idx < num_blocks:
                # 增加一个 min 限制，防止 prompt_block_num_list 和 dict_list 长度不一致导致越界
                valid_index = min(i, len(text_context_dict_list) - 1)
                return text_context_dict_list[valid_index]
            
            # 否则减去当前段的长度，继续往后寻找
            current_idx -= num_blocks

        return text_context_dict_list[-1]
    ...
    def inference_with_trajectory_inference(
        ...
        video_output = []
        audio_output = []
        ROLLOUT_CACHE_BLOCK_IDX = 6 # (video_latent_num_frames - 4) // 3 + 1 - 1 # 6
        ...
        for block in tqdm(blocks, desc="Inference blocks"):
            ...
            # prompt switch
            text_context_dict = self.switch_text_context_dict(block_idx, prompt_block_num_list, text_context_dict_list)
    ...
    def adapt_batch(
        ...
        # --- 1. 图像处理与潜变量生成 (所有 prompt 共享，只计算一次) ---
        ...
        # --- 2. 文本编码：复用 unconditional 字典，仅遍历 prompt_list ---
        conditional_dict_list = []
        unconditional_dict = None
        nospeech_v, nospeech_a = None, None  # 缓存 nospeech 的特征
        ...
        with torch.no_grad():
            for prompt in prompt_list:
                ...
                # 提取当前正向 prompt 特征
                v_context_p = ctx_p.video_encoding.to(self.device)
                a_context_p = ctx_p.audio_encoding.to(self.device)
                
                # 如果是第一次循环，初始化并缓存 unconditional 和 nospeech 特征
                if unconditional_dict is None:
                    v_context_n = ctx_n.video_encoding.to(self.device)
                    a_context_n = ctx_n.audio_encoding.to(self.device)
                    ...
                    nospeech_v = ctx_nospeech.video_encoding.to(self.device)
                    nospeech_a = ctx_nospeech.audio_encoding.to(self.device)

                # 处理 no_speech 拼接
                if use_nospeech_prompt:
                    v_context_p = torch.cat([v_context_p, nospeech_v], dim=1)
                    a_context_p = torch.cat([a_context_p, nospeech_a], dim=1)

                # 将当前 prompt 结果存入列表
                conditional_dict_list.append({
        ...
        # --- 3. 组装最终的单一 Batch ---
        new_batch = {
            "conditional_dict_list": conditional_dict_list, # 包含所有 prompt 条件的列表
            "unconditional_dict": unconditional_dict,       # 只计算一次的负向条件
    ...
    def _parse_prompt_list(self, prompt_list: list[dict]):
        """
        解析 prompt_list，返回 prompt_list_str, prompt_block_num_list, total_frame_num, max_chunk_frame_num
        prompt_list_str: prompt 列表，每个元素为 prompt 字符串
        prompt_block_num_list: 每个 prompt block数列表
        total_frame_num: 总帧数 (inference时计算总block数)
        max_chunk_frame_num: 最大 chunk 帧数 (adapt batch只需使用最大 chunk 帧数)
        """
        prompt_list_str = [] # prompts
        prompt_block_num_list = [] # 每个 prompt block数
        total_frame_num = 0 # 总帧数
        max_chunk_frame_num = 0 # 最大 chunk 帧数
        for item in prompt_list:
            # new_item = {}
            prompt_list_str.append(item["content"])
            prompt_block_num_list.append(round(item["len"]))
            total_frame_num += round(item["len"]) * 24
            max_chunk_frame_num = max(max_chunk_frame_num, round(item["len"]) * 24)
        total_frame_num += 1
        max_chunk_frame_num += 1
        print(f"total_frame_num={total_frame_num}, max_chunk_frame_num={max_chunk_frame_num}")
        return prompt_list_str, prompt_block_num_list, total_frame_num, max_chunk_frame_num
    ...
    def generate(
        ...
        prompt_list_str, prompt_block_num_list, total_frame_num, max_chunk_frame_num = \
                self._parse_prompt_list(prompt_list)
        ...
        with torch.no_grad():
            ...
            info_dict = self.adapt_batch(
                image_path, 
                prompt_list_str, 
                num_inference_frames=max_chunk_frame_num, 
                resolution=resolution,
                frame_rate=frame_rate)
            # infer data in actual latent num frames
            info_dict['video_latent_num_frames_output'] = (total_frame_num - 1) // 8 + 1
            video_pred, audio_pred = \
                self.inference_with_trajectory_inference(info_dict['initial_video_latent_state'],
                                            ...
                                            text_context_dict_list=info_dict['conditional_dict_list'],
                                            prompt_block_num_list=prompt_block_num_list)
            self.write_video(
                video_pred, 
                audio_pred, 
                prompt=prompt_list_str[0],
                video_num_frames=total_frame_num, 
