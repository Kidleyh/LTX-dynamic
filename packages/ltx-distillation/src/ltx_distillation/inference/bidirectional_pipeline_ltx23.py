"""
Bidirectional Audio-Video Trajectory Pipeline for DMD backward simulation.

This pipeline generates denoising trajectories for backward simulation
in DMD training. It runs the generator through multiple denoising steps
and returns the intermediate states.
"""

from typing import Tuple, Dict, Any, Optional
import torch
import torch.nn as nn
from ltx_core.types import LatentState

from ltx_core.model.transformer.modality import Modality

class LTX23BidirectionalAVTrajectoryPipeline:
    """
    Pipeline for generating audio-video denoising trajectories.

    Used in DMD training for backward simulation:
    1. Start from pure noise
    2. Denoise through multiple steps using the generator
    3. Return trajectory of intermediate states

    The trajectory can be used to sample training inputs at different noise levels.
    """

    def __init__(
        self,
        generator: nn.Module,
        add_noise_fn,
        denoising_sigmas: torch.Tensor,
        device,
    ):
        """
        Args:
            generator: LTX2DiffusionWrapper instance
            add_noise_fn: Callable[[original, noise, sigma], noisy_sample]
                         Flow matching noise addition: (1-sigma)*x0 + sigma*eps
            denoising_sigmas: Tensor of sigma values for denoising steps
        """
        self.generator = generator
        self.add_noise_fn = add_noise_fn
        self.denoising_sigmas = denoising_sigmas
        # if self.denoising_sigmas[-1] <= 0.0001:
        #     self.denoising_sigmas = self.denoising_sigmas[:-1]  # remove the zero timestep for inference
        self.param_dtype = next(self.generator.parameters()).dtype
        self.device = device

    def inference_generator_with_grad(
        self,
        video_latent_state: LatentState,
        audio_latent_state: LatentState,
        video_latent_num_frames: int,
        sigma: torch.Tensor,
        conditional_dict: Dict[str, Any],
        ) -> Tuple[torch.Tensor, torch.Tensor]:
        B = video_latent_state.latent.shape[0]
        F_v = video_latent_num_frames # video_noise.shape[1]
        token_length_v = video_latent_state.latent.shape[1]
        token_length_a = audio_latent_state.latent.shape[1]

        # [NOTE]: DEVICE AND DTYPE NOT INITIALIZED
        video_noisy_input = video_latent_state.latent.to(device=self.device, dtype=self.param_dtype)
        audio_noisy_input = audio_latent_state.latent.to(device=self.device, dtype=self.param_dtype)
        video_denoise_mask = video_latent_state.denoise_mask.to(device=self.device, dtype=self.param_dtype)

        batch_size, video_seqlen, feat_dim = video_latent_state.latent.shape 
        batch_size, audio_seqlen, feat_dim = audio_latent_state.latent.shape

        video_latent_model_input = video_noisy_input * video_denoise_mask + video_latent_state.clean_latent * (1 - video_denoise_mask)
        audio_latent_model_input = audio_noisy_input
        video_timesteps = sigma * video_denoise_mask
        audio_timesteps = sigma * audio_latent_state.denoise_mask.to(device=self.device, dtype=self.param_dtype)
        
        video_modality = Modality(
            latent=video_latent_model_input.to(device=self.device, dtype=self.param_dtype),
            sigma=sigma.repeat(batch_size).to(device=self.device, dtype=self.param_dtype),
            timesteps=video_timesteps.to(device=self.device, dtype=self.param_dtype),
            positions=video_latent_state.positions.to(device=self.device, dtype=self.param_dtype),  
            context=conditional_dict['v_context_p'].to(device=self.device, dtype=self.param_dtype),
            enabled=True,
            context_mask=None,
            attention_mask=None,
        )
        audio_modality = Modality(
            latent=audio_latent_model_input.to(device=self.device, dtype=self.param_dtype),
            sigma=sigma.repeat(batch_size).to(device=self.device, dtype=self.param_dtype),
            timesteps=audio_timesteps.to(device=self.device, dtype=self.param_dtype),
            positions=audio_latent_state.positions.to(device=self.device, dtype=self.param_dtype), 
            context=conditional_dict['a_context_p'].to(device=self.device, dtype=self.param_dtype), 
            enabled=True,
            context_mask=None,
            attention_mask=None,
        )

        # Predict x0
        pred_video, pred_audio = self.generator(
            video_modality,
            audio_modality,
            perturbations=None,
        )

        return pred_video, pred_audio

    @torch.no_grad()
    def inference_with_trajectory(
        self,
        video_latent_state: LatentState,
        audio_latent_state: LatentState,
        video_latent_num_frames: int,
        text_context_dict: Dict[str, Any],
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Generate denoising trajectory from noise.

        This implements consistency backward simulation:
        At each step, predict x0 and then re-corrupt to the next noise level.

        Args:
            video_noise: Initial video noise [B, F_v, C, H, W]
            audio_noise: Initial audio noise [B, F_a, C]
            text_context_dict: Conditioning dictionary

        Returns:
            Tuple of:
                - video_trajectory: [B, T, F_v, C, H, W] where T is num steps
                - audio_trajectory: [B, T, F_a, C]
        """
        B = video_latent_state.latent.shape[0]
        F_v = video_latent_num_frames # video_noise.shape[1]
        token_length_v = video_latent_state.latent.shape[1]
        token_length_a = audio_latent_state.latent.shape[1]
        # device = video_latent_state.latent.device

        # noisy_video = video_noise
        # noisy_audio = audio_noise

        # [NOTE]: DEVICE AND DTYPE NOT INITIALIZED
        video_denoise_mask = video_latent_state.denoise_mask.to(device=self.device, dtype=self.param_dtype)
        fresh_noise_video = torch.randn_like(video_latent_state.latent)
        fresh_noise_audio = torch.randn_like(audio_latent_state.latent)
        video_noisy_input = fresh_noise_video * video_denoise_mask + video_latent_state.clean_latent * (1 - video_denoise_mask) # video_latent_state.latent.to(device=self.device, dtype=self.param_dtype)
        audio_noisy_input = fresh_noise_audio # audio_latent_state.latent.to(device=self.device, dtype=self.param_dtype)

        # sigma == 1的输入
        video_trajectory = []
        audio_trajectory = []

        for i, sigma in enumerate(self.denoising_sigmas[:-1]):
            
            video_trajectory.append(video_noisy_input)
            audio_trajectory.append(audio_noisy_input)

            # Prepare sigma tensors
            torch.cuda.empty_cache()
            batch_size, video_seqlen, feat_dim = video_noisy_input.shape 
            batch_size, audio_seqlen, feat_dim = audio_noisy_input.shape

            video_latent_model_input = video_noisy_input * video_denoise_mask + video_latent_state.clean_latent * (1 - video_denoise_mask)
            audio_latent_model_input = audio_noisy_input
            video_timesteps = sigma * video_denoise_mask
            audio_timesteps = sigma * audio_latent_state.denoise_mask.to(device=self.device, dtype=self.param_dtype)
            
            video_modality = Modality(
                latent=video_latent_model_input.to(device=self.device, dtype=self.param_dtype),
                sigma=sigma.repeat(batch_size).to(device=self.device, dtype=self.param_dtype),
                timesteps=video_timesteps.to(device=self.device, dtype=self.param_dtype),
                positions=video_latent_state.positions.to(device=self.device, dtype=self.param_dtype),  
                context=text_context_dict['v_context'].to(device=self.device, dtype=self.param_dtype),
                enabled=True,
                context_mask=None,
                attention_mask=None,
            )
            audio_modality = Modality(
                latent=audio_latent_model_input.to(device=self.device, dtype=self.param_dtype),
                sigma=sigma.repeat(batch_size).to(device=self.device, dtype=self.param_dtype),
                timesteps=audio_timesteps.to(device=self.device, dtype=self.param_dtype),
                positions=audio_latent_state.positions.to(device=self.device, dtype=self.param_dtype), 
                context=text_context_dict['a_context'].to(device=self.device, dtype=self.param_dtype), 
                enabled=True,
                context_mask=None,
                attention_mask=None,
            )

            # Predict x0

            pred_video, pred_audio = self.generator(
                video_modality,
                audio_modality,
                perturbations=None,
            )

            # Get next sigma
            next_sigma = self.denoising_sigmas[i + 1]

            if next_sigma > 0:
                # Re-corrupt with next sigma level
                # For flow matching: x_t = (1 - sigma) * x_0 + sigma * eps
                # We need to add noise at the next sigma level

                # Sample fresh noise
                fresh_noise_video = torch.randn_like(pred_video)
                fresh_noise_audio = torch.randn_like(pred_audio)

                next_video_sigma = next_sigma * torch.ones([B, token_length_v], device=self.device)
                next_audio_sigma = next_sigma * torch.ones([B, token_length_a], device=self.device)

                video_noisy_input = self.add_noise_fn(
                    pred_video.flatten(0, 1),
                    fresh_noise_video.flatten(0, 1),
                    next_video_sigma.flatten(0, 1),
                ).unflatten(0, (B, token_length_v))

                audio_noisy_input = self.add_noise_fn(
                    pred_audio, fresh_noise_audio, next_audio_sigma
                )
            else:
                # At t=0, just use the prediction
                video_noisy_input = pred_video
                audio_noisy_input = pred_audio

            # video_trajectory.append(video_noisy_input)
            # audio_trajectory.append(audio_noisy_input)

        # Stack trajectories: [B, T, F, C, H, W](omniforcing implementation) -> [B, T(num_sigmas), L(token_per_batch), C](omniforcing implementation)
        # tracjectories内容: sigma步数0/1/2对应的输入，以及sigma步数==2的预测输出(sigma_val == 0)，共3+1 == 4个东西
        video_trajectory = torch.stack(video_trajectory, dim=1)
        audio_trajectory = torch.stack(audio_trajectory, dim=1)

        # video_trajectory = video_trajectory.view(B, len(self.denoising_sigmas), video_latent_num_frames, -1, video_trajectory.shape[-1])

        return video_trajectory, audio_trajectory


class BidirectionalAVInferencePipeline:
    """
    Pipeline for few-step bidirectional inference.

    Used for validation after training to generate videos/audio
    using the distilled model.
    """

    def __init__(
        self,
        generator: nn.Module,
        add_noise_fn,
        denoising_sigmas: torch.Tensor,
    ):
        """
        Args:
            generator: Distilled LTX2DiffusionWrapper
            add_noise_fn: Callable[[original, noise, sigma], noisy_sample]
            denoising_sigmas: Sigma values for few-step denoising
        """
        self.generator = generator
        self.add_noise_fn = add_noise_fn
        self.denoising_sigmas = denoising_sigmas

    @torch.no_grad()
    def generate(
        self,
        video_shape: Tuple[int, ...],
        audio_shape: Tuple[int, ...],
        conditional_dict: Dict[str, Any],
        seed: Optional[int] = None,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Generate video and audio using few-step denoising.

        Args:
            video_shape: (B, F_v, C, H, W) video latent shape
            audio_shape: (B, F_a, C) audio latent shape
            conditional_dict: Text conditioning
            seed: Random seed (optional)

        Returns:
            Tuple of (video_latent, audio_latent)
        """
        B = video_shape[0]
        F_v = video_shape[1]
        F_a = audio_shape[1]

        # Set seed if provided
        if seed is not None:
            torch.manual_seed(seed)

        device = next(self.generator.parameters()).device
        dtype = next(self.generator.parameters()).dtype

        # Initialize with noise
        video = torch.randn(video_shape, device=device, dtype=dtype)
        audio = torch.randn(audio_shape, device=device, dtype=dtype)

        # Few-step denoising
        for i, sigma in enumerate(self.denoising_sigmas[:-1]):
            video_sigma = sigma * torch.ones([B, F_v], device=device)
            audio_sigma = sigma * torch.ones([B, F_a], device=device)

            # Predict x0
            pred_video, pred_audio = self.generator(
                noisy_image_or_video=video,
                conditional_dict=conditional_dict,
                timestep=video_sigma,
                noisy_audio=audio,
                audio_timestep=audio_sigma,
            )

            # Get next sigma
            next_sigma = self.denoising_sigmas[i + 1]

            if next_sigma > 0:
                # Euler step or re-corruption
                fresh_noise_video = torch.randn_like(video)
                fresh_noise_audio = torch.randn_like(audio)

                next_video_sigma = next_sigma * torch.ones([B, F_v], device=device)
                next_audio_sigma = next_sigma * torch.ones([B, F_a], device=device)

                video = self.add_noise_fn(
                    pred_video.flatten(0, 1),
                    fresh_noise_video.flatten(0, 1),
                    next_video_sigma.flatten(0, 1),
                ).unflatten(0, (B, F_v))

                audio = self.add_noise_fn(
                    pred_audio, fresh_noise_audio, next_audio_sigma
                )
            else:
                video = pred_video
                audio = pred_audio

        return video, audio
