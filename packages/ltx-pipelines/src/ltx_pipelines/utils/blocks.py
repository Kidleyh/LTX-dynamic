...
# packages/ltx-causal/src/ltx_causal/transformer/causal_model.py
...
class DiffusionStage:
    ...
    def _build_transformer(self, *, device: torch.device | None = None, **kwargs: object) -> X0Model:
        ...
        if self.causal:
            return CausalX0Model(builder.build(device=target, **kwargs)).to(target).eval()
        else:
    ...
    def __call__(  # noqa: PLR0913
        ...
        if video is not None:
            ...
            # [NOTE]: We set the original shape here to allow for operation later
            video_state = replace(video_state, original_shape=v_shape)
        ...
        if audio is not None:
            ...
            # [NOTE]: We set the original shape here to allow for operation later
            audio_state = replace(audio_state, original_shape=a_shape)

        if gen_ode:
            if self.base_transformer is None:
                self.base_transformer = self._build_transformer(video_tools=video_tools)
                # "/gemini/platform/public/aigc/human_guozz2/code/hys/LTX-2/ltx_experiments/0415_ltx_node_bs21_bidmd_smalllr/checkpoint_model_005000/model_gen.pt"
                if trained_ckpt_path is not None:
                    model_state_dict = torch.load(trained_ckpt_path)
                    new_state_dict = {k[6:] if k.startswith('model.') else k: v for k, v in model_state_dict['generator'].items()}
                    self.base_transformer.load_state_dict(new_state_dict)
                    print("checkpoint loaded:", trained_ckpt_path)
                self.base_transformer = self.base_transformer.to(self._device)
        ...
        if not gen_ode:
            ...
            # transformer = BatchSplitAdapter(self.base_transformer, max_batch_size=max_batch_size)
            video_state, audio_state, (video_every_latents, audio_every_latents) = loop(
                sigmas=sigmas,
                video_state=video_state,
                audio_state=audio_state,
                stepper=stepper,
                transformer=self.base_transformer,
                denoiser=denoiser,
        ...
        if gen_ode:
            return video_state, audio_state, (video_every_latents, audio_every_latents)
